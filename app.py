...
def health():
    # --- HEALTH_CACHE_AGE_V1 ---
    now_ts = _now_ts()
    last_reload = float(_CACHE_RELOAD_LAST_COMPLETE_TS or 0.0)
...
CASE_TRACKING_SHEET_NAME = "CASE_TRACKING"
CASE_MESSAGE_LOG_SHEET_NAME = "CASE_MESSAGE_LOG"
CASE_TRACKING_HEADERS = [
...
CASE_MESSAGE_LOG_HEADERS = [
    "log_id", "case_id", "created_at", "sender_role", "user_id", "intent", "message_text", "trace_id", "event_key",
]
CASE_OPEN_STATUSES = {"open", "assigned", "waiting_user", "waiting_admin"}
CASE_TERMINAL_STATUSES = {"resolved", "closed"}
CASE_TRACKING_MATCH_WINDOW_SECONDS = int(os.getenv("CASE_TRACKING_MATCH_WINDOW_SECONDS", "259200").strip() or "259200")
CASE_TRACKING_PRIVATE_ONLY = os.getenv("CASE_TRACKING_PRIVATE_ONLY", "1").strip().lower() not in {"0", "false", "no"}
...
ALERT_MANAGER_PUSH_TEXT = os.getenv(
    "ALERT_MANAGER_PUSH_TEXT",
    "⚠️ Cảnh báo sự cố trong group. Nội dung đã được dịch trong nhóm. Vui lòng kiểm tra ngay."
).strip() or "⚠️ Cảnh báo sự cố trong group. Nội dung đã được dịch trong nhóm. Vui lòng kiểm tra ngay."
_ALERT_MANAGER_PUSH_RATE_LIMIT_CACHE: Dict[str, float] = {}
_ALERT_MANAGER_PUSH_RATE_LIMIT_LOCK = threading.Lock()
...
# --- ROUTING_MISS_ALERT_V1 ---
_ROUTING_MISS_COUNTER = 0
_ROUTING_MISS_ALERT_LOCK = threading.Lock()
_ROUTING_MISS_LAST_ALERT_TS = 0.0
ROUTING_MISS_ALERT_THRESHOLD = int(os.getenv("ROUTING_MISS_ALERT_THRESHOLD", "20").strip() or "20")
ROUTING_MISS_ALERT_COOLDOWN_SECONDS = int(os.getenv("ROUTING_MISS_ALERT_COOLDOWN_SECONDS", "3600").strip() or "3600")

def _increment_routing_miss_counter(trace_id: str) -> None:
    """
    In-memory routing miss counter.
    Does not read Google Sheet.
    Emits ROUTING_MISS_ALERT_THRESHOLD_HIT in Render logs when threshold is reached.
    """
    global _ROUTING_MISS_COUNTER, _ROUTING_MISS_LAST_ALERT_TS
    with _ROUTING_MISS_ALERT_LOCK:
        _ROUTING_MISS_COUNTER += 1
        count = _ROUTING_MISS_COUNTER
        elapsed = _now_ts() - float(_ROUTING_MISS_LAST_ALERT_TS or 0.0)
        if count >= ROUTING_MISS_ALERT_THRESHOLD and elapsed >= ROUTING_MISS_ALERT_COOLDOWN_SECONDS:
            logger.warning(
                f"[{trace_id}] ROUTING_MISS_ALERT_THRESHOLD_HIT "
                f"miss_count={count} threshold={ROUTING_MISS_ALERT_THRESHOLD} "
                f"cooldown_elapsed={int(elapsed)}s "
                f"ACTION_REQUIRED=check_ROUTING_MISS_HARVEST_and_update_INTENT_MASTER"
            )
            _ROUTING_MISS_COUNTER = 0
            _ROUTING_MISS_LAST_ALERT_TS = _now_ts()
...
def append_routing_miss_event(
    ...
    _increment_routing_miss_counter(trace_id)
    return queued
...
def try_build_routing_reply(text: str, language_group: str, trace_id: str, user_id: str = "", source_type: str = "user") -> Optional[dict]:
    ...
    if intent_name == "chat_general":
        logger.info(
            f"[{trace_id}] ROUTING_CHAT_GENERAL_EARLY_RETURN "
            f"matched_keywords={json.dumps(matched_keywords, ensure_ascii=False)}"
        )
        return {
            "reply_text": t(language_group, "default_general_intent"),
            "intent_name": intent_name,
            "service_row": {},
            "location_hint": "",
            "location_id": "",
...
def group_safe_routing_result_allows_reply(routing_result: Optional[dict]) -> bool:
    ...
    return result_type in ("routed", "sim_fastpath") and bool(service_row)
...
# --- ALERT_MANAGER_PUSH_V1 ---
def push_line_text(to_user_id: str, text: str, trace_id: str) -> bool:
    """
    Send privacy-minimized LINE push message to one user.
    Raw LINE userId is used only in API payload; logs expose user_ref only.
    """
    target_user_id = safe_str(to_user_id)
    if not ALERT_MANAGER_PUSH_ENABLED:
        logger.info(f"[{trace_id}] ALERT_MANAGER_PUSH_FAILED reason=disabled")
        return False
    if not LINE_CHANNEL_ACCESS_TOKEN:
        logger.error(f"[{trace_id}] ALERT_MANAGER_PUSH_FAILED reason=missing_access_token")
        return False
    if not target_user_id:
        logger.error(f"[{trace_id}] ALERT_MANAGER_PUSH_FAILED reason=missing_target")
        return False

    body_text = safe_str(text)[:LINE_TEXT_HARD_LIMIT] or ALERT_MANAGER_PUSH_TEXT
...
def _alert_manager_incident_rate_key(tenant_id: str, source_ref_value: str, incident_fp: str) -> str:
    return "::".join([
        safe_str(tenant_id) or "unknown_tenant",
        safe_str(source_ref_value) or "unknown_source",
        safe_str(incident_fp) or "unknown_incident",
    ])
...
def alert_manager_is_rate_limited(trace_id: str, tenant_id: str, source_ref_value: str, incident_fp: str) -> bool:
    ...
    with _ALERT_MANAGER_PUSH_RATE_LIMIT_LOCK:
        last_ts = float(_ALERT_MANAGER_PUSH_RATE_LIMIT_CACHE.get(key, 0.0) or 0.0)
        elapsed = now_ts - last_ts if last_ts else 0.0
        if last_ts and elapsed < ALERT_MANAGER_PUSH_RATE_LIMIT_SECONDS:
            logger.warning(
                f"[{trace_id}] ALERT_MANAGER_PUSH_RATE_LIMITED "
                f"tenant_id={safe_str(tenant_id)} source_ref={safe_str(source_ref_value)} "
                f"incident_fp={safe_str(incident_fp)} elapsed_seconds={int(elapsed)} "
...
def alert_manager_mark_rate_limit(trace_id: str, tenant_id: str, source_ref_value: str, incident_fp: str) -> None:
    ...
    with _ALERT_MANAGER_PUSH_RATE_LIMIT_LOCK:
        ...
        if len(_ALERT_MANAGER_PUSH_RATE_LIMIT_CACHE) > RATE_LIMIT_STORE_MAX_KEYS:
            oldest = sorted(
                _ALERT_MANAGER_PUSH_RATE_LIMIT_CACHE.items(),
...
def resolve_alert_manager_targets(event: dict, trace_id: str) -> List[str]:
    """
    Resolve push targets from ROLE_MAP + ADMIN_IDS.

    ROLE_MAP stores user_ref only for privacy. LINE Push API requires raw userId.
    Therefore V1 only pushes to raw ADMIN_IDS whose user_ref matches active ROLE_MAP
    role in ALERT_MANAGER_TARGET_ROLES. No raw ID is logged.
    """
    try:
        source_id, actual_source_type = get_source_id_from_event(event)
        source_ref_value = stable_hash(source_id) if source_id else ""
        ...
        admin_ref_to_user_id = {
            user_ref(raw_user_id): raw_user_id
            for raw_user_id in ADMIN_LIST
            if safe_str(raw_user_id)
        }
        rows = load_routing_master_records(
            trace_id, GROUP_CONTEXT_ROLE_MAP_SHEET_NAME, GROUP_CONTEXT_ROLE_CACHE_TTL_SECONDS
        )
        targets: List[str] = []
        role_ref_hits = 0
        role_ref_missing_raw_target = 0
        for row in rows or []:
            row_tenant_id = safe_str(row.get("tenant_id"))
            row_source_type = safe_str(row.get("source_type")).lower()
            row_source_ref = safe_str(row.get("source_ref"))
            row_user_ref = safe_str(row.get("user_ref"))
            row_role = normalize_group_context_role(row.get("role"))
            row_status = safe_str(row.get("status")).lower()
            if row_tenant_id != tenant_id:
                continue
            if row_source_type != actual_source_type:
            ...
            if row_status and row_status != "active":
                continue
            if row_role not in ALERT_MANAGER_TARGET_ROLES:
                continue
            role_ref_hits += 1
            raw_target = admin_ref_to_user_id.get(row_user_ref)
            if not raw_target:
                role_ref_missing_raw_target += 1
                continue
            if raw_target not in targets:
                targets.append(raw_target)

        logger.info(
            f"[{trace_id}] ALERT_MANAGER_TARGETS_RESOLVED "
            f"tenant_id={tenant_id} source_type={actual_source_type} source_ref={source_ref_value} "
            f"role_ref_hits={role_ref_hits} push_targets={len(targets)} "
            f"missing_raw_target={role_ref_missing_raw_target}"
        )
        return targets
    except Exception as exc:
        logger.exception(f"[{trace_id}] ALERT_MANAGER_TARGETS_RESOLVE_FAILED exception={type(exc).__name__}:{exc}")
        return []


def maybe_push_alert_manager(event: dict, trace_id: str, raw_text: str) -> bool:
    """
    Push manager alert only after COST_GUARD decision confirms active incident.
    Does not infer alert from keywords alone.
    """
    decision = get_group_context_decision()
    action = safe_str(decision.get("action"))
    if action != "alert_manager_translate":
        return False
    if not (
        bool(decision.get("risk_keyword_detected"))
        and safe_str(decision.get("event_time_context")) == "current_or_imminent"
        and safe_str(decision.get("speaker_intent")) == "report_or_request_help"
    ):
        logger.info(
            f"[{trace_id}] ALERT_MANAGER_PUSH_FAILED "
            f"reason=decision_context_mismatch action={action} "
            f"event_time_context={safe_str(decision.get('event_time_context'))} "
            f"speaker_intent={safe_str(decision.get('speaker_intent'))}"
        )
        return False
    ...
    if not targets:
        logger.warning(
            f"[{trace_id}] ALERT_MANAGER_PUSH_SKIPPED_NO_TARGET "
            f"tenant_id={safe_str(tenant_id)} source_ref={safe_str(source_ref_value)} incident_fp={incident_fp}"
        )
        return False
...
def should_open_case(intent: str, normalized_text: str) -> bool:
    intent = (intent or "").strip().lower()
    text = (normalized_text or "").strip().lower()

    if intent in {"leave", "health"}:
        return True

    if intent == "travel":
        travel_followup_phrases = [
            "cần báo", "theo mẫu nào", "mẫu nào",
            "perlu lapor", "perlu isi formulir", "formulir apa",
            "ต้องแจ้ง", "ต้องลางานไหม", "แจ้งไหม",
        ]
        return any(p in text for p in travel_followup_phrases)

    return False


def case_intent_group(intent: str) -> str:
    normalized = safe_str(intent).lower()
    if normalized in {"leave", "health"}:
        return "leave_health"
...
def ensure_case_tracking_worksheet(trace_id: str):
    ...
    if not spreadsheet:
        return None
    try:
        ws = spreadsheet.worksheet(CASE_TRACKING_SHEET_NAME)
    except gspread.WorksheetNotFound:
        try:
            ws = spreadsheet.add_worksheet(title=CASE_TRACKING_SHEET_NAME, rows=2000, cols=len(CASE_TRACKING_HEADERS) + 2)
            append_row_guarded(ws, trace_id, CASE_TRACKING_SHEET_NAME, CASE_TRACKING_HEADERS, value_input_option="USER_ENTERED")
            logger.info(f"[{trace_id}] CASE_TRACKING_SHEET_CREATED worksheet_name={CASE_TRACKING_SHEET_NAME}")
            return ws
        except Exception as e:
            logger.exception(f"[{trace_id}] CASE_TRACKING_SHEET_CREATE_FAILED exception={type(e).__name__}:{e}")
            return None
    except Exception as e:
        logger.exception(f"[{trace_id}] CASE_TRACKING_SHEET_OPEN_FAILED exception={type(e).__name__}:{e}")
        return None
    values = get_all_values_safe(ws, trace_id, CASE_TRACKING_SHEET_NAME)
    if not values:
        try:
            append_row_guarded(ws, trace_id, CASE_TRACKING_SHEET_NAME, CASE_TRACKING_HEADERS, value_input_option="USER_ENTERED")
            logger.info(f"[{trace_id}] CASE_TRACKING_HEADERS_INIT_OK")
        except Exception as e:
            logger.exception(f"[{trace_id}] CASE_TRACKING_HEADERS_INIT_FAILED exception={type(e).__name__}:{e}")
            return None
    return ws
...
def ensure_case_message_log_worksheet(trace_id: str):
    ...
    if not spreadsheet:
        return None
    try:
        ws = spreadsheet.worksheet(CASE_MESSAGE_LOG_SHEET_NAME)
    except gspread.WorksheetNotFound:
        try:
            ws = spreadsheet.add_worksheet(title=CASE_MESSAGE_LOG_SHEET_NAME, rows=5000, cols=len(CASE_MESSAGE_LOG_HEADERS) + 2)
            append_row_guarded(ws, trace_id, CASE_MESSAGE_LOG_SHEET_NAME, CASE_MESSAGE_LOG_HEADERS, value_input_option="USER_ENTERED")
            logger.info(f"[{trace_id}] CASE_MESSAGE_LOG_SHEET_CREATED worksheet_name={CASE_MESSAGE_LOG_SHEET_NAME}")
            return ws
        except Exception as e:
            logger.exception(f"[{trace_id}] CASE_MESSAGE_LOG_SHEET_CREATE_FAILED exception={type(e).__name__}:{e}")
            return None
    except Exception as e:
        logger.exception(f"[{trace_id}] CASE_MESSAGE_LOG_SHEET_OPEN_FAILED exception={type(e).__name__}:{e}")
        return None
    values = get_all_values_safe(ws, trace_id, CASE_MESSAGE_LOG_SHEET_NAME)
    if not values:
        try:
            append_row_guarded(ws, trace_id, CASE_MESSAGE_LOG_SHEET_NAME, CASE_MESSAGE_LOG_HEADERS, value_input_option="USER_ENTERED")
            logger.info(f"[{trace_id}] CASE_MESSAGE_LOG_HEADERS_INIT_OK")
        except Exception as e:
            logger.exception(f"[{trace_id}] CASE_MESSAGE_LOG_HEADERS_INIT_FAILED exception={type(e).__name__}:{e}")
            return None
    return ws


def _make_case_id() -> str:
    return f"CASE_{datetime.now(TW_TZ).strftime('%Y%m%d_%H%M%S')}_{uuid.uuid4().hex[:6].upper()}"


def _make_case_log_id() -> str:
    return f"CASELOG_{datetime.now(TW_TZ).strftime('%Y%m%d_%H%M%S')}_{uuid.uuid4().hex[:6].upper()}"


def find_open_case_for_user(user_id: str, intent: str, trace_id: str) -> Optional[dict]:
    scoped_user_id = tenant_scope_key(user_id)
    if not scoped_user_id:
        return None
    ws = ensure_case_tracking_worksheet(trace_id)
    if not ws:
        logger.error(f"[{trace_id}] CASE_TRACKING_LOOKUP_SKIPPED reason=worksheet_unavailable")
        return None
    records = get_records_safe(ws, trace_id, CASE_TRACKING_SHEET_NAME)
    if not records:
        return None
    expected_group = case_intent_group(intent)
    now_dt = now_tw_dt()
    matched = None
    matched_dt = None
    for row in records:
        if safe_str(row.get("user_id")) != scoped_user_id:
            continue
        status = safe_str(row.get("status")).lower()
        if status not in CASE_OPEN_STATUSES:
            continue
        row_group = case_intent_group(row.get("intent"))
        if row_group != expected_group:
            continue
        updated_dt = parse_iso_datetime(row.get("updated_at")) or parse_iso_datetime(row.get("created_at"))
        if not updated_dt:
            continue
        if int((now_dt - updated_dt).total_seconds()) > CASE_TRACKING_MATCH_WINDOW_SECONDS:
            continue
        if matched_dt is None or updated_dt > matched_dt:
            matched = row
            matched_dt = updated_dt
    if matched:
        logger.info(f"[{trace_id}] CASE_MATCH_FOUND case_id={safe_str(matched.get('case_id'))} intent_group={expected_group}")
    return matched
...
def create_case_record(user_id: str, language_group: str, intent: str, raw_text: str, normalized_text: str, source_channel: str, trace_id: str) -> Optional[str]:
    ...
    row = [
        ...
        now_iso,
        now_iso,
        tenant_scope_key(user_id),
        normalize_language_group(language_group),
        safe_str(intent).lower(),
        truncate_text(raw_text, 500),
        truncate_text(normalized_text, 500),
        "open",
        "unassigned",
        safe_str(source_channel) or "user",
        "normal",
        now_iso,
        "",
        "",
    ...
    try:
        append_row_guarded(ws, trace_id, CASE_TRACKING_SHEET_NAME, row, value_input_option="USER_ENTERED")
        _invalidate_worksheet_caches(CASE_TRACKING_SHEET_NAME)
        logger.info(f"[{trace_id}] CASE_CREATED case_id={case_id} intent={safe_str(intent).lower()} status=open")
        return case_id
    except Exception as e:
        logger.exception(f"[{trace_id}] CASE_CREATE_FAILED exception={type(e).__name__}:{e}")
        return None


def append_case_message_log(case_id: str, sender_role: str, user_id: str, intent: str, message_text: str, trace_id: str, event_key: str = "") -> bool:
    ws = ensure_case_message_log_worksheet(trace_id)
    if not ws:
        logger.error(f"[{trace_id}] CASE_MESSAGE_LOG_APPEND_SKIPPED reason=worksheet_unavailable case_id={case_id}")
        return False
    row = [
        _make_case_log_id(),
        safe_str(case_id),
        now_tw_iso(),
        safe_str(sender_role) or "user",
        tenant_scope_key(user_id),
        safe_str(intent).lower(),
        truncate_text(message_text, 500),
        trace_id,
        safe_str(event_key) or get_current_event_ref(),
    ]
    try:
        append_row_guarded(ws, trace_id, CASE_MESSAGE_LOG_SHEET_NAME, row, value_input_option="USER_ENTERED")
        _invalidate_worksheet_caches(CASE_MESSAGE_LOG_SHEET_NAME)
        logger.info(f"[{trace_id}] CASE_MESSAGE_LOG_APPEND_OK case_id={case_id} sender_role={safe_str(sender_role) or 'user'}")
        return True
    except Exception as e:
        logger.exception(f"[{trace_id}] CASE_MESSAGE_LOG_APPEND_FAILED case_id={case_id} exception={type(e).__name__}:{e}")
        return False


def touch_case_tracking(case_id: str, trace_id: str, status: str = "") -> bool:
    ws = ensure_case_tracking_worksheet(trace_id)
    if not ws:
        return False
    row_index = find_first_row_index_by_column_value(ws, "case_id", case_id, trace_id, CASE_TRACKING_SHEET_NAME)
    if row_index <= 0:
        logger.error(f"[{trace_id}] CASE_UPDATE_ROW_NOT_FOUND case_id={case_id}")
        return False
    now_iso = now_tw_iso()
    fields = {"updated_at": now_iso, "last_user_message_at": now_iso, "trace_id": trace_id}
    if safe_str(status):
        fields["status"] = safe_str(status)
    ok = update_row_fields_by_header(ws, row_index, fields, trace_id, CASE_TRACKING_SHEET_NAME, identity_column="case_id", identity_value=case_id)
    if ok and safe_str(status):
        logger.info(f"[{trace_id}] CASE_STATUS_UPDATED case_id={case_id} to={safe_str(status)}")
    return ok
...
def attach_or_create_case(user_id: str, language_group: str, intent: str, raw_text: str, normalized_text: str, source_channel: str, trace_id: str, event_key: str = "") -> Optional[str]:
    if CASE_TRACKING_PRIVATE_ONLY and not is_private_source_type(source_channel):
        logger.info(f"[{trace_id}] CASE_TRACKING_SKIPPED reason=non_private_source source_channel={safe_str(source_channel)}")
        return None
    if not should_open_case(intent, normalized_text):
        logger.info(f"[{trace_id}] CASE_TRACKING_SKIPPED reason=rule_not_matched intent={safe_str(intent).lower()}")
        return None
    matched = find_open_case_for_user(user_id, intent, trace_id)
    if matched:
        case_id = safe_str(matched.get("case_id"))
        if case_id:
            touch_case_tracking(case_id, trace_id)
            append_case_message_log(case_id, "user", user_id, intent, raw_text, trace_id, event_key=event_key)
            logger.info(f"[{trace_id}] CASE_MATCHED_EXISTING case_id={case_id} matched_by=user_id+intent_group")
            return case_id
    case_id = create_case_record(user_id, language_group, intent, raw_text, normalized_text, source_channel, trace_id)
    if case_id:
        append_case_message_log(case_id, "user", user_id, intent, raw_text, trace_id, event_key=event_key)
    return case_id
...
MT_EMPTY_TRANSLATION_REPLY_TEXT = os.getenv(
    "MT_EMPTY_TRANSLATION_REPLY_TEXT",
    "Vui lòng gửi nội dung cần dịch sau lệnh."
).strip()
MT_SOURCE_TYPE_MISMATCH_REPLY_TEXT = os.getenv(
    ...
    for row in source_rows:
        ...
        if source_id.lower().startswith("ref:"):
            ...
            if source_ref_value:
                ...
                source_meta[f"ref:{source_ref_value}"] = meta
                source_ref_meta[source_ref_value] = meta
        else:
...
def mt_prepare_translation_request(raw_text: str, tenant: dict) -> dict:
    """
    SAAS_HARDENING_V3:
    - Tách phần payload cần dịch khỏi câu lệnh tự nhiên, ví dụ: "Tôi muốn dịch: ...".
    - Tôn trọng lệnh explicit target như /zh, /vi, /tr zh.
    - Không tự suy diễn thuật ngữ; chỉ chuẩn hóa phần command wrapper.
    """
    raw = sanitize_incoming_text(raw_text)
    parsed = parse_translation_command(raw)
    if parsed.get("is_translation"):
        content = sanitize_incoming_text(parsed.get("content"))
        target_override = safe_str(parsed.get("target_lang"))
        command = safe_str(parsed.get("command")) or "translation_command"
        return {
            "content": content,
    ...
    natural_patterns = [
        r"^\s*(?:tôi\s+muốn\s+d[iị]ch|toi\s+muon\s+dich|hãy\s+d[iị]ch|hay\s+dich|d[iị]ch|translate)\s*[:：\-]\s*(.+)$",
        r"^\s*(?:phiên\s+d[iị]ch|phien\s+dich)\s*[:：\-]\s*(.+)$",
    ]
    for pattern in natural_patterns:
        match = re.match(pattern, raw, flags=re.IGNORECASE)
        if match:
            return {
                "content": sanitize_incoming_text(match.group(1)),
                "target_override": "",
                "command": "natural_payload_prefix",
                "is_command": True,
            }
    ...
    return {
        "content": raw,
        "target_override": "",
        "command": "auto_raw_text",
        "is_command": False,
    }


def build_mt_direction_meta_for_explicit_target(target_lang: str, trace_id: str = "") -> dict:
    target = safe_str(target_lang)
    result = {
        "target_lang": target,
        "direction": f"explicit->{target}",
        "confidence": "explicit",
        "reason": "explicit_translation_command",
        "features": {},
    }
    logger.info(
        f"[{trace_id}] MT_DIRECTION_DECISION target_lang={target} "
        f"direction={result['direction']} confidence=explicit reason=explicit_translation_command"
    )
    return result
...
def should_handle_mt_translation(event: dict, trace_id: str) -> Tuple[bool, dict]:
    ...
    if MT_SOURCE_TYPE_STRICT_ENABLED and configured_source_type in {"user", "group", "room"} and configured_source_type != source_type:
        logger.warning(
            f"[{trace_id}] MT_SOURCE_TYPE_MISMATCH tenant_id={tenant_id} source_ref={source_ref_value} "
            f"configured_source_type={configured_source_type} actual_source_type={source_type} lookup_mode={lookup_mode}"
        )
        return False, {
...
def handle_mt_translation_message(event: dict, trace_id: str) -> Optional[dict]:
    ...
    if not should_handle:
        ...
        if reason == "source_type_mismatch":
            ...
            reply_ok = reply_line_text(get_reply_token(event), MT_SOURCE_TYPE_MISMATCH_REPLY_TEXT, trace_id, "vi")
            enqueue_mt_translation_log({
                "timestamp": now_tw_iso(),
                "trace_id": trace_id,
                "tenant_id": safe_str(ctx.get("tenant_id")),
                "source_ref": safe_str(ctx.get("source_ref")),
                "source_type": safe_str(ctx.get("source_type")),
                "user_ref": user_ref(get_event_user_id(event)),
                "target_lang": "",
                "direction": "blocked_source_type_mismatch",
                "direction_confidence": "guard",
                "direction_reason": f"configured={safe_str(ctx.get('configured_source_type'))};actual={safe_str(ctx.get('source_type'))}",
    ...
    raw_text = get_message_text(event)
    translation_request = mt_prepare_translation_request(raw_text, tenant)
    ...
    # Explicit translation commands keep priority so "dịch câu: tôi cần sim" still translates.
    if is_group_safe_source_type(source_type) and not parse_translation_command(raw_text).get("is_translation"):
        service_hint = group_context_has_service_keyword_hint(raw_text)
        if service_hint:
            logger.info(
                f"[{trace_id}] MT_SERVICE_INTENT_GUARD_CHECK "
                f"source_type={safe_str(source_type)} service_hint=True text_fp={message_fingerprint(raw_text)}"
            )
            service_result = handle_service_routing_before_mt(
                event=event,
                trace_id=trace_id,
                user_id=user_id,
                ...
                text=raw_text,
                source_type=source_type,
                force_service_check=True,
            )
            if service_result:
                logger.info(
                    f"[{trace_id}] MT_SERVICE_INTENT_GUARD_HANDLED "
                    f"flow_used={safe_str(service_result.get('flow_used'))} reply_sent={bool(service_result.get('reply_sent'))}"
                )
                return service_result
            logger.info(
                f"[{trace_id}] MT_SERVICE_INTENT_GUARD_SILENT "
                f"reason=service_hint_no_routing_result text_fp={message_fingerprint(raw_text)}"
            )
            return build_group_safe_silent_result(user_id, "mt_service_intent_guard_no_routing_result")
    ...
    if not translation_text:
        reply_ok = reply_line_text(reply_token, MT_EMPTY_TRANSLATION_REPLY_TEXT, trace_id, "vi")
        enqueue_mt_translation_log({
            "timestamp": now_tw_iso(),
            "trace_id": trace_id,
            "tenant_id": tenant_id,
            ...
            "user_ref": user_ref(user_id),
            "target_lang": "",
            "direction": "blocked_empty_payload",
            "direction_confidence": "guard",
            "direction_reason": translation_command or "empty_translation_payload",
            "raw_text_fp": message_fingerprint(raw_text),
            "glossary_text_fp": "",
            "translated_len": "0",
            "quota_before": "",
    ...
    if MT_MAX_INPUT_CHARS > 0 and len(translation_text) > MT_MAX_INPUT_CHARS:
        reply_ok = reply_line_text(reply_token, MT_INPUT_TOO_LONG_REPLY_TEXT, trace_id, "vi")
        enqueue_mt_translation_log({
            "timestamp": now_tw_iso(),
            "trace_id": trace_id,
            "tenant_id": tenant_id,
            ...
            "user_ref": user_ref(user_id),
            "target_lang": "",
            "direction": "blocked_input_too_long",
            "direction_confidence": "guard",
            "direction_reason": f"len={len(translation_text)} max={MT_MAX_INPUT_CHARS}",
            "raw_text_fp": message_fingerprint(raw_text),
            "glossary_text_fp": "",
            "translated_len": "0",
            "quota_before": "",
    ...
    try:
        ...
        # Direction Guard chạy trên translation_text đã bóc command wrapper, trước khi apply glossary.
        # Ví dụ: "Tôi muốn dịch: hôm nay tôi cần tăng ca" -> detect trên "hôm nay tôi cần tăng ca".
        if explicit_target_lang:
            target_lang = explicit_target_lang
            direction_meta = build_mt_direction_meta_for_explicit_target(target_lang, trace_id)
        else:
            direction_meta = resolve_mt_target_language_with_meta(translation_text, tenant, trace_id)
            target_lang = safe_str(direction_meta.get("target_lang"))
        glossary_text = apply_glossary(translation_text, tenant_id)
        if not target_lang:
        ...
        logger.info(
            ...
            f"translated_len={len(translated)} quota_before={quota_before} quota_after={quota_after} reply_ok={reply_ok} command={translation_command}"
        )
    except Exception as exc:
...
def handle_service_routing_before_mt(event: dict, trace_id: str, user_id: str, reply_token: str, text: str, source_type: str, force_service_check: bool = False) -> Optional[dict]:
    ...
    if is_group_safe_source_type(source_type) and COST_GUARD_CONTEXT_CLASSIFIER_ENABLED and not force_service_check:
    ...
    service_keyword = has_service_keyword_for_routing(text, trace_id) if (service_hint or force_service_check or not is_group_safe_source_type(source_type)) else False
    if not service_keyword:
        return None
...
def dispatch_text_event(event: dict, trace_id: str) -> dict:
    ...
    if normalized == WORKER_ENTRY_COMMAND:
        ...
        if is_cjk_text(text) and not has_service_keyword_for_routing(text, trace_id):
            ...
            if routing_result:
                ...
                reply_text = build_default_intent_reply(text, current_language, trace_id)
                flow_used = "default"
                # --- PRIVATE_UNHANDLED_FALLBACK_V1 ---
                # Khi private message không route được và rơi vào default intent,
                # emit log riêng để forensic, không thay đổi reply_text.
                if flow_used == "default" and is_private_source_type(source_type):
                    logger.warning(
                        f"[{trace_id}] PRIVATE_UNHANDLED_FALLBACK "
                        f"user_ref={user_ref(user_id)} "
                        f"text_fp={message_fingerprint(text)} "
                        f"flow_used=default source_type={safe_str(source_type)}"
                    )
    ...
    default_intent_details = None
    if flow_used in {"default", "ads_auto_cleared"}:
        try:
            default_intent_details = resolve_default_intent_details(normalized)
            attach_or_create_case(
                user_id=user_id,
                language_group=reply_language,
                intent=safe_str(default_intent_details.get("intent")) or "general",
                raw_text=text,
                normalized_text=safe_str(default_intent_details.get("normalized")) or normalized,
                source_channel=source_type,
                trace_id=trace_id,
                event_key=get_current_event_ref(),
            )
        except Exception as e:
            logger.exception(f"[{trace_id}] CASE_TRACKING_ATTACH_FAILED exception={type(e).__name__}:{e}")
